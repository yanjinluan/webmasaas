...
export default {
    name:'HaveTenantAdmin',
    props:['changeIsShow', 'tenantObj'],
    data () {
        return{
            userLists:[],
            loading:true
    ...
    methods:{
        getUsers () {
            //获取角色的用户列表
            this.$http.get(`/api/application/tenants/${this.tenantObj.id}/administrators/`,{
                params:{
        ...
        deleteAdmin (userId) {
            //删除角色下的用户
            this.$http.delete(`/api/application/tenants/${this.tenantObj.id}/administrators/${userId}/?applicationId=${this.applicationId}`, {
                headers:{
                    'Operate-Application-Id': this.applicationId
                }
            }).then( res => {
