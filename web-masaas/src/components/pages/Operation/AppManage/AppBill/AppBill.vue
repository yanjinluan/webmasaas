            ...
            >
                ...
                >
                    <template slot-scope="scope">
                        <el-button size='mini' type="warning" plain @click="onBillWindow(scope.row.id,'detailbill', scope.row.appId)">查看</el-button>
                        <el-button size='mini' type="primary" plain @click="download(scope.row.id, scope.row.appId)">导出</el-button>
                        <el-button size='mini' type="info" plain @click="onBillWindow(scope.row.id, 'appbilldetail', scope.row.appId)">详情列表</el-button>
                    </template>
                </el-table-column>
            </el-table>
        </div> 
...
export default {
    ...
    methods:{
        ...
        fuzzyLists () {//模糊查询数据
            this.loading = true;
            let { pageNumber, pageSize, fuzzyBillPeriod, fuzzyTenantName, fuzzyApplicationId } = this;
            let billPeriod = this.filterTime (fuzzyBillPeriod);
            this.$http.get(`/api/application/bills/?applicationId=${fuzzyApplicationId}`,{
                params:{
                    pageSize,
                    pageNumber,
                    time:new Date().getTime(),
					tenantName:fuzzyTenantName,
                    billPeriod
                },
                headers:{
        ...
        fuzzyClick () {//点击查询按钮时
            ...
            this.loading = true;
            this.pageNumber = 1;
			let { pageNumber, pageSize, billPeriod, tenantName, applicationId } = this;
            ...
            this.$http.get(`/api/application/bills/?applicationId=${applicationId}`,{
                params:{
                    pageSize,
                    pageNumber,
                    tenantName,
        ...
        onBillWindow (billId, type, appId) {
        ...
        download (billId) {
            //导出
            this.$http.get(`/api/application/bills/${billId}/statement/`,{
